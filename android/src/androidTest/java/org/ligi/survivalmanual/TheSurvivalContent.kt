...
class TheSurvivalContent {
    ...
    private val survivalContent = SurvivalContent(InstrumentationRegistry.getTargetContext().assets)

    @Test
    fun weCanLoadAllEntriesFromNavigation() {
        navigationEntryMap.forEach { it ->
            ...
            val unresolvedLinks = elementCollectingVisitor.elementList.map {
                it.getTextInNode(tested).toString()
