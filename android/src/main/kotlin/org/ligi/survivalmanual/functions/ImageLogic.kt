...
fun isImage(fileName: String) = fileName.endsWith(".png") ||
        fileName.endsWith(".jpg") || fileName.endsWith(".vd")

fun getSurvivalDrawable(ctx: Context, source: String) = if (source.endsWith(".vd")) {
    when (source.substringBeforeLast(".")) {
        "med_open_airway" -> VectorDrawableCompat.create(ctx.resources,
                R.drawable.ic_med_open_airway, null)
